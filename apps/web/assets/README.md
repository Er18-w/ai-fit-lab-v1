# Demo 图片来源

本目录照片仅用于产品原型演示，均来自标注为免费使用的 Unsplash 或 Pexels 页面。商品名称、规格和匹配分数是独立的 Demo 假数据，不代表照片中的真实商品或品牌。

- `product-tshirt.jpg`: Unsplash / Mediamodifier, photo `elbKS4DY21g`
- `product-flatlay.jpg`: Pexels / cottonbro studio, photo `3927390`
- `product-sneaker.jpg`: Pexels / Mikhail Nilov, photo `2529148`
- `product-jeans.jpg`: Pexels / Mica Asato, photo `1082529`
- `product-shirts.jpg`: Pexels / Kai Pilger, photo `996329`
- `hair-short.jpg`: Pexels / Bimbim Sindu, photo `10745863`
- `hair-curly.jpg`: Pexels / Built By King, photo `9928018`
- `hair-long.jpg`: Pexels / Joseilson De lima, photo `36784935`
- `hair-red.jpg`: Pexels / Christina Chekhomova, photo `11054052`
- `editorial-autumn-woman.jpg`: Unsplash, photo `1733379056005-24b474d8f8a1`

The `wardrobe-*` files are temporary product-reference images collected from
Pinterest search results on 2026-09-16 for this non-commercial prototype. They
are deliberately labelled as demo data in the UI and must be replaced with
merchant-provided, user-owned, or explicitly licensed assets before release.

对外商业发布前应再次核对届时有效的图库许可、人物肖像使用条件与署名要求，并用品牌自有或取得明确授权的商品素材替换。
